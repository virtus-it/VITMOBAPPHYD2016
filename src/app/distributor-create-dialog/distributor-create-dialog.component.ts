...
export class DistributorCreateDialogComponent implements OnInit {
    dist = { firstName: "", lastName: "", phone: '', mobile1: "", mobile2: "", companyname: "", address: "", emailid: "", referCode: "", selectedItems: [], phonetype: "", gstno: "", acceptonlinepayment: false };
    areaList = [];
    phone = false;
    ...
    onSubmit() {
        ...
        if (this.dist.gstno) {
            if (this.dist.gstno.length <= 50) {
                input.User['gstnumber'] = this.dist.gstno;
            } else {
                this.validateMessage = "Only 50 characters allowed in GST number";
                return;
            }
        }
        AuthenticationService.showLog("Input : " + JSON.stringify(input));
        //console.log(input);
